...
1.	Discord Webhook
	If you want Discord notifications, copy your webhook URL.
	Open sqliX_demo.py (or app.py, whichever you named it) and set the DISCORD_WEBHOOK_URL variable:
    ```python
   DISCORD_WEBHOOK_URL = "https://discord.com/api/webhooks/your_webhook_id/your_webhook_token"
    ```
    If you leave DISCORD_WEBHOOK_URL = "", no alerts will be sent.
2.	Port and Debug Mode
    By default, the Flask app runs on port 5000 with debug mode enabled.
    To change the port, edit the last lines:
    ```python
...
Uses eight regular expressions to catch common SQLi syntax (numeric comparisons, built-in functions, SQL comments, keywords, etc.).
